...
namespace Assets.TouchStuff
{
    ...
    public class JSwipeManager : MonoBehaviour
    {
        ...
        {
            var touches = JInputHelper.GetTouches();
            if (touches.Count > 0)
            {
                Touch t = touches[0];
        ...
        void SetCurrentSwipeDirection()
        {
            var deltaPos = currentSwipeAction.endPosition - currentSwipeAction.startPosition;
            var dx = deltaPos.x;
            var dy = deltaPos.y;
            ...
            currentSwipeAction.direction = direction;
        }
